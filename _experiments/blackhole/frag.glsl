...
{   
    ...
    float distFromOrigin = length(point.xz);
    vec2 orbitPoint = vec2(cos(time*0.70)*4., sin(time*0.70)*1.5);
    float distFromOrbit = dot(point.xz - orbitPoint, point.xz - orbitPoint);
    vec4 mapping = vec4(1., 1., 1., 
        point.y 
        + .5
        + sin(pow(distFromOrigin, 1.8)*5.-time)/5. / (distFromOrigin*distFromOrigin+1.)
        - exp(-distFromOrbit)/2.*(sin(time)/2.+1.));
    mapping = combine(mapping, vec4(0,0,0,sdfSphere(p, vec3(0, .8 + sin(time+0.8)/3., 0), .7)));
    mapping = combine(mapping, vec4(0.95, .5, .5, sdfSphere(p, vec3(orbitPoint.x, .8 + sin(time)/3., orbitPoint.y), 0.2)));
    return mapping;
}
...
vec3 render(vec3 rayOrigin, vec3 rayDirection)
{
    ...
    for(i=0; i < 200; i++)
    {
        ...
        // Because we're warping the domain, the SDF 
        rayProgress += mapping.w * 0.8;
    ...
    {
        vec3 normal = calcNormalTetra(rayPosition);
        colour = mapping.xyz * max(0., dot(normal, vec3(0,1.,0))*0.5 + 0.5);
    }
    else return vec3(1.);
